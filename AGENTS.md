...
| Location stocktake and undo | `InventoryWorkflows.tsx`: LocationStocktake, OperationHistoryDialog | Location tree opens a count dialog for the selected location; recent transactions opens undo; App refreshes via `load` | `/stocktake/:locationId`, `/stocktake`, `/stock/operations`, `/stock/operations/:id/undo`; `workflows.ts`, operation snapshots in `stock.ts` | API `workflows.test.ts`, `mcp-workflow.test.ts` |
