...
| Location stocktake and undo | `InventoryWorkflows.tsx`: StocktakeDialog, OperationHistoryDialog | App opens dialogs and refreshes via `load`; dialogs submit locally | `/stocktake/:locationId`, `/stocktake`, `/stock/operations`, `/stock/operations/:id/undo`; `workflows.ts`, operation snapshots in `stock.ts` | API `workflows.test.ts`, `mcp-workflow.test.ts` |
