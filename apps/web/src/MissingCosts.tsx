...
export function MissingCosts({homeId,revision,onSaved}:{homeId:string;revision:unknown;onSaved:()=>void}) {
  ...
  return <section className="panel missing-cost-panel" aria-labelledby="missing-cost-title">
    ...
    {error?<p className="empty" role="alert">{error} <button type="button" onClick={()=>setRefresh(value=>value+1)}>{t("重试")}</button></p>:<>
      <div className="missing-cost-list" aria-busy={loading}>{data?.items.map(batch=><div className="missing-cost-entry" key={batch.batchId}><div><strong>{batch.itemName}</strong><small>{batch.receivedAt.slice(0,10)}{batch.label&&batch.label!=="历史库存"?` · ${batch.label}`:""} · {t(batch.legacy?"历史累计入库数量":"原始入库数量")} {batch.quantity} {displayUnit(batch.unit)} · {t("当前剩余数量")} {batch.remainingQuantity} {displayUnit(batch.unit)}{batch.issueCount>0?` · ${t("关联 {{count}} 笔出库",{count:batch.issueCount})}`:""}</small>{batch.quantity<=0&&<small>{t("入库数量异常，需先核对库存记录")}</small>}</div><button type="button" className="text-button" disabled={loading||batch.quantity<=0} onClick={()=>setEditing(batch)}>{t("补录成本")}</button></div>)}</div>
