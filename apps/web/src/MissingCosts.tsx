...
type MissingBatch={batchId:string;itemId:string;itemName:string;unit:string;label:string|null;receivedAt:string;quantity:number;issueCount:number};
...
export function MissingCosts({homeId,revision,onSaved}:{homeId:string;revision:unknown;onSaved:()=>void}) {
  ...
  return <section className="panel missing-cost-panel" aria-labelledby="missing-cost-title">
    ...
    {error?<p className="empty" role="alert">{error} <button type="button" onClick={()=>setRefresh(value=>value+1)}>{t("重试")}</button></p>:<>
      <div className="missing-cost-list" aria-busy={loading}>{data?.items.map(batch=><div className="missing-cost-entry" key={batch.batchId}><div><strong>{batch.itemName}</strong><small>{batch.receivedAt.slice(0,10)} · {batch.label||batch.batchId.slice(0,8)} · {t("入库数量")} {batch.quantity} {displayUnit(batch.unit)}{batch.issueCount>0?` · ${t("关联 {{count}} 笔出库",{count:batch.issueCount})}`:""}</small>{batch.quantity<=0&&<small>{t("入库数量异常，需先核对库存记录")}</small>}</div><button type="button" className="text-button" disabled={loading||batch.quantity<=0} onClick={()=>setEditing(batch)}>{t("补录成本")}</button></div>)}</div>
