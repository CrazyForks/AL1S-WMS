...
export function BatchFields({
  ...
  const [date, setDate] = useState(manufacturedDate),
    [amount, setAmount] = useState(""),
    [unit, setUnit] = useState("day"),
    [expiry, setExpiry] = useState(expiryDate);
  function calculate(nextDate: string, nextAmount: string, nextUnit: string) {
    ...
    if (nextUnit === "day") result.setDate(result.getDate() + value);
      ...
      result.setDate(1);
      result.setMonth(
        result.getMonth() + value * (nextUnit === "year" ? 12 : 1),
      );
      const last = new Date(
        result.getFullYear(),
        result.getMonth() + 1,
  ...
  return (
    <fieldset className="batch-dates">
      <legend>{title}</legend>
        ...
        <label>
          ...
          <input
            ...
            onChange={(event) => {
              ...
              calculate(event.target.value, amount, unit);
            }}
          />
        </label>
        <label>
          ...
          <div className="form-row">
            <input
              ...
              onChange={(event) => {
                ...
                calculate(date, event.target.value, unit);
              }}
            />
            <select
              aria-label={t("保质期单位")}
              ...
              onChange={(event) => {
                ...
                calculate(date, amount, event.target.value);
              }}
            >
