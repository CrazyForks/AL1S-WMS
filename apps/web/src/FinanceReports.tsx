...
function useInventoryRange(defaultMode="12"){
  const currentMonth=new Date().toISOString().slice(0,7);
  const [mode,setMode]=useState(defaultMode);
  const [customStart,setCustomStart]=useState(adjacentMonth(currentMonth,-11));
  const [customEnd,setCustomEnd]=useState(currentMonth);
  const end=mode==="custom"?customEnd:mode==="previous"?adjacentMonth(currentMonth,-1):currentMonth;
  const start=mode==="custom"?customStart:mode==="ytd"?`${currentMonth.slice(0,4)}-01`:mode==="current"||mode==="previous"?end:adjacentMonth(currentMonth,1-Number(mode));
  const months=(Number(end.slice(0,4))-Number(start.slice(0,4)))*12+Number(end.slice(5))-Number(start.slice(5))+1;
  const valid=/^\d{4}-(0[1-9]|1[0-2])$/.test(start)&&/^\d{4}-(0[1-9]|1[0-2])$/.test(end)&&months>0&&months<=36;
  return {mode,setMode,customStart,setCustomStart,customEnd,setCustomEnd,start,end,valid};
}
function InventoryRangeControls({range,kind}:{range:ReturnType<typeof useInventoryRange>;kind:"receipt"|"waste"}){
  const {t}=useTranslation();
  const presets=kind==="receipt"?[["6","近6个月"],["12","近12个月"],["ytd","本年"],["custom","自定义"]]:[["current","本月"],["previous","上月"],["6","近6个月"],["12","近12个月"],["custom","自定义"]];
  return <div className="report-controls"><strong className="inventory-range-label">{t(kind==="receipt"?"入库时间":"浪费发生时间")}</strong><div className="report-presets">{presets.map(([value,label])=><button type="button" key={value} aria-pressed={range.mode===value} onClick={()=>range.setMode(value)}>{t(label)}</button>)}</div>
    {range.mode==="custom"&&<div className="report-range"><label>{t("开始月份")}<input type="month" value={range.customStart} onChange={event=>range.setCustomStart(event.target.value)}/></label><label>{t("结束月份")}<input type="month" value={range.customEnd} onChange={event=>range.setCustomEnd(event.target.value)}/></label></div>}
    {range.valid&&<small className="report-range-caption">{range.start} — {range.end}</small>}
  </div>;
}
function InventoryReportStatus({valid,report}:{valid:boolean;report:ReturnType<typeof useReport<InventoryCostReport>>}){
  const {t}=useTranslation();
...
function InventoryCohorts({homeId,revision}:InventoryReportProps){
  ...
  const [grain,setGrain]=useState<CohortGrain>("month");
  const [percent,setPercent]=useState(false);
  const valid=range.valid&&!!asOf;
  const report=useReport<InventoryCostReport>(valid?`/api/v1/homes/${homeId}/inventory-cost-analysis?start=${range.start}&end=${range.end}&granularity=month&asOf=${asOf}`:"",revision);
  const data=report.data;
...
function InventoryWasteRanking({homeId,revision,onOpenItem}:InventoryReportProps){
  const {t,i18n}=useTranslation();
  const range=useInventoryRange("6");
  const [view,setView]=useState<"item"|"category"|"location">("item");
  const report=useReport<InventoryCostReport>(range.valid?`/api/v1/homes/${homeId}/inventory-cost-analysis?start=${range.start}&end=${range.end}&granularity=month`:"",revision);
...
export function InventoryCostWaste(props:InventoryReportProps){
  ...
  return <section className="inventory-cost-section" aria-labelledby="inventory-cost-title">
    ...
    <div className="inventory-analysis-grid"><InventoryWasteRanking {...props}/><InventoryExpiryRisk {...props}/></div>
  </section>;
}
