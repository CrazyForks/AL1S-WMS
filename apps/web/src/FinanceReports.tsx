    ...
    <section className="panel inventory-cost-panel">
      ...
      {!valid?<p className="empty" role="alert">{t("请选择不超过36个月的有效范围")}</p>:report.error?<p className="empty" role="alert">{report.error} <button onClick={report.retry}>{t("重试")}</button></p>:report.loading||!data?<p className="empty" role="status">{t("加载中…")}</p>:<>
        ...
        <div className="cost-grid">
          ...
          <section className="cost-insight-card"><div className="cost-card-head"><div><h3>{t("浪费构成")}</h3><p>{t("过期与损坏视为纯损耗，库存调整单列")}</p></div></div><dl><div><dt>{t("过期")}</dt><dd>{money(data.totals.expired)}</dd></div><div><dt>{t("损坏")}</dt><dd>{money(data.totals.damaged)}</dd></div></dl><p className="cost-risk-note">{t("出库损耗占比按期间浪费除以正常使用与浪费之和计算，不代表采购批次浪费率")}</p></section>
