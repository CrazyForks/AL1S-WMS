...
import type {Item,Stock,OpenedConsumable,Location,TransactionPage,ShoppingItem,ShoppingChannel,FinancialSummary,Category} from "./webTypes.js";
...
export async function getOpenedConsumables() {
  return readHome<OpenedConsumable[]>(`opened-consumables`, "无法加载已开封消耗品");
}
...
export async function getTransactions(page = 1, snapshotAt = "", pageSize = 10) {
  return readHome<TransactionPage>(`transactions?limit=${pageSize}&offset=${(page - 1) * pageSize}${snapshotAt ? `&snapshotAt=${encodeURIComponent(snapshotAt)}` : ""}`, "无法加载变动记录");
}
...
export async function getShoppingCalendar(month: string, includeCompleted = false) {
  return readHome<ShoppingItem[]>(`shopping-calendar?month=${month}&includeCompleted=${includeCompleted}`, "无法加载采购日历");
}

export async function getFinancialSummary(month:string) {
  return readHome<FinancialSummary>(`financial-dashboard?month=${month}`, "无法加载财务数据");
}
