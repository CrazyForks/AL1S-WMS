import { ArrowRight,Bot,Cat,Dog,Trash2,UserRound,Venus } from "lucide-react";
...
export const unitOptions = ["个","瓶","盒","包","箱","袋","罐","桶","卷","支","根","条","片","张","块","颗","把","双","套","份","克","市斤","千克","毫升","升","厘米","米","其他"] as const;
...
export const avatarOptions: {value:UserAvatar;label:string;Icon:typeof UserRound}[] = [
...
export function AvatarIcon({avatar,size=16}:{avatar?:UserAvatar|null;size?:number}) {
  const Icon=avatarOptions.find(option=>option.value===avatar)?.Icon??UserRound;
  return <Icon size={size} aria-hidden="true"/>;
}

export function TransactionRow({ transaction,onOpenItem }: { transaction: Transaction;onOpenItem?:(itemId:string)=>void }) {
  const labels = {
    receipt: t("入库"),
    issue: t("领用"),
    delete: t("删除"),
