...
export function App() {
  ...
  async function updateItem(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/v1/homes/${getHomeId()}/items/${detailItem.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: data.get("name"), baseUnit: data.get("baseUnit"), reorderPoint: Number(data.get("reorderPoint") || 0), locationId: data.get("locationId") || null, manufacturedDate: data.get("manufacturedDate") || null, expiryDate: data.get("expiryDate") || null }) });
  ...
  return <div className="shell">
    ...
    <main>
      ...
      {!countView && <section className="content-grid">
      ...
      {countView && <section className="panel count-panel"><div className="panel-head"><div><h2>地点盘点</h2><p className="muted">展开地点，核对实际库存</p></div><button className="text-button" onClick={() => setCountView(false)}>返回清单</button></div>{locations.map((location) => { const open = expandedLocations[location.id] ?? true; const locationItems = items.filter((item) => item.locationId === location.id); return <div className="count-location" key={location.id}><button className="count-location-head" onClick={() => setExpandedLocations({ ...expandedLocations, [location.id]: !open })}><span>{open ? "⌄" : "›"} {location.name}</span><strong>{locationItems.reduce((sum, item) => sum + balanceFor(item.id), 0)}</strong></button>{open && locationItems.map((item) => <div className="count-item" key={item.id}><span>{item.name}</span><span>{balanceFor(item.id)} {item.baseUnit}</span><button className="text-button" onClick={() => setDetailItem(item)}>编辑</button></div>)}</div>; })}</section>}
    </main>
    {detailItem && <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setDetailItem(null)}><form className="modal" onSubmit={updateItem}><div className="modal-head"><div><h2>编辑物资</h2><p className="muted">{detailItem.name}</p></div><button type="button" className="close" onClick={() => setDetailItem(null)} aria-label="关闭"><X size={18} strokeWidth={1.8} /></button></div><label>物资名称<input name="name" required defaultValue={detailItem.name} /></label><div className="form-row"><label>单位<input name="baseUnit" required defaultValue={detailItem.baseUnit} /></label><label>最低库存<input name="reorderPoint" type="number" min="0" step="0.1" defaultValue={detailItem.reorderPoint} /></label></div><div className="form-row"><label>生产日期<input name="manufacturedDate" type="date" defaultValue={detailItem.manufacturedDate || ""} /></label><label>保质期至<input name="expiryDate" type="date" defaultValue={detailItem.expiryDate || ""} /></label></div><label>存放地点<select name="locationId" defaultValue={detailItem.locationId || ""}><option value="">暂不指定</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select></label><button className="primary full">保存修改</button></form></div>}
