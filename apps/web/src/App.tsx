        ...
        {activePage === "finance" && financeDashboard && (
          <div className="finance-page">
            ...
            <section className="finance-layout">
              <FinanceTrend key={getHomeId()} homeId={getHomeId()} month={financeMonth} revision={financeDashboard}/>
        ...
        {!countView && !logView && activePage === "count" && (
          <section className="count-workspace">
            ...
            <section className="panel opened-consumables-panel">
              ...
              {openedConsumables.length===0?<p className="empty">{t("暂无已开封消耗品")}</p>:<div className="table-wrap"><table><thead><tr><th>{t("物资")}</th><th>{t("数量")}</th><th>{t("地点")}</th><th>{t("开封后到期")}</th><th>{t("保质期")}</th><th>{t("开封时间")}</th><th>{t("操作")}</th></tr></thead><tbody>{openedConsumables.map(opened=><tr key={opened.id}><td><button type="button" className="item-link" onClick={()=>openItemDetail(opened.itemId)}>{opened.itemName}</button></td><td>{opened.quantity} {displayUnit(opened.baseUnit)}</td><td>{opened.locationName||t("未指定")}</td><td>{opened.openedExpiryDate||t("未设置")}</td><td>{opened.expiryDate||t("未设置")}</td><td>{new Date(opened.openedAt).toLocaleString(localeForDates(),{year:"numeric",month:"numeric",day:"numeric",hour:"2-digit",minute:"2-digit"})}</td><td><button type="button" className="text-button" disabled={busy} onClick={()=>setExhaustTarget(opened)}>{t("用尽")}</button></td></tr>)}</tbody></table></div>}
            </section>
