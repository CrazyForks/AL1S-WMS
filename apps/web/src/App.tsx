          ...
          <div className="finance-page">
            ...
            <section className="finance-layout">
              ...
              <form className="panel finance-budget" onSubmit={saveFinanceBudget}>
                ...
                <div className="finance-budget-body"><div className="finance-budget-flow"><svg className="budget-links" viewBox="0 0 640 230" preserveAspectRatio="none" aria-hidden="true"><path d="M 26 116 C 180 116,260 36,440 36"/><path d="M 26 116 C 180 116,260 80,440 80"/><path d="M 26 116 C 180 116,260 124,440 124"/><path d="M 26 116 C 180 116,260 168,440 168"/></svg><div className="finance-budget-overview"><span className="budget-origin" aria-hidden="true"/><label>{t("月度总预算")}<input value={financeBudgetTotal} onChange={event=>setFinanceBudgetTotal(event.target.value)} type="number" min="0" step="0.01" placeholder="0.00" /></label><div className="budget-allocation"><span>{t("已分配")}</span><strong>{formatMoney(financeAllocatedBudget,financeDashboard.currency)}</strong><small>{t("可分配")} {financeBudgetTotal.trim()===""?t("未设置"):formatMoney(Number(financeBudgetTotal)-financeAllocatedBudget,financeDashboard.currency)}</small></div></div><div className="finance-budget-editor"><div className="budget-editor-head"><strong>{t("分类预算")}</strong><small>{t("按分类预留本月采购额度")}</small></div><div className="budget-category-picker"><select value={financeCategorySelection} onChange={event=>setFinanceCategorySelection(event.target.value)}><option value="">{t("选择分类")}</option>{financeCategoryOptions.map(category=><option key={category.id} value={category.name}>{"　".repeat(category.depth)}{category.name}</option>)}</select><input value={financeCategoryAmount} onChange={event=>setFinanceCategoryAmount(event.target.value)} type="number" min="0" step="0.01" placeholder={t("预算金额")} /><button type="button" className="secondary" disabled={!financeCategorySelection||financeCategoryAmount.trim()===""} onClick={addFinanceCategoryBudget}>{t("添加")}</button></div><div className="category-budget-list">{financeBudgetEntries.length?financeBudgetEntries.map((entry,index)=><div className="category-budget-entry" key={entry.category}><i className="budget-entry-anchor" aria-hidden="true"/><span>{entry.category}</span><input value={entry.amount} onChange={event=>setFinanceBudgetEntries(entries=>entries.map((value,current)=>current===index?{...value,amount:event.target.value}:value))} type="number" min="0" step="0.01" /><button type="button" aria-label={t("移除{{name}}",{name:entry.category})} onClick={()=>setFinanceBudgetEntries(entries=>entries.filter((_,current)=>current!==index))}><X size={14}/></button></div>):<p className="muted">{t("尚未添加分类预算")}</p>}</div></div></div><div className="finance-budget-actions"><small>{t("分类预算合计不得超过总预算")}</small><button className="primary" disabled={financeSaving}>{financeSaving?t("保存中…"):t("保存预算")}</button></div></div>
              </form>
