            ...
            <section className="finance-layout">
              <section className="panel finance-trend">
                ...
                <div className="trend-chart" aria-label={t("过去 12 个月支出趋势")}>
                  {financeDashboard.trend.map(point=>{
                    const max=spendingTrendScale(financeDashboard.trend);
                    ...
                    const tooltip=[point.month,`${t("月度预算")} ${point.budget===null?t("未设置"):formatMoney(point.budget,financeDashboard.currency)}`,`${t("已花")} ${formatMoney(point.actual,financeDashboard.currency)}`,`${t("待采购")} ${formatMoney(point.planned,financeDashboard.currency)}`,`${remaining!==null&&remaining<0?t("超支"):t("剩余")} ${remaining===null?t("未设置"):formatMoney(Math.abs(remaining),financeDashboard.currency)}`].join("\n");
