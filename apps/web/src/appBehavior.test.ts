...
import {appInternals} from "./testing/appInternals.js";

const paths={home:"/",count:"/count",shopping:"/shopping",finance:"/finance",locations:"/locations",categories:"/categories",profile:"/profile"};
test("navigation preserves trailing slash, unknown path and detail source behavior",()=>{
  const browser={location:{pathname:"/"},history:{state:null as null|{itemDetailSource:unknown}}};
  const api=appInternals(["pageFromUrl","itemDetailIdFromUrl","itemDetailSourcePage"],{window:browser,pagePaths:paths});
...
test("opened expiry ignores invalid values and compares date-only values at local end of day",()=>{
  const api=appInternals(["formatDateTime","openedExpiryForDisplay"]);
...
test("read requests preserve paths, paging, snapshot encoding and endpoint errors",async()=>{
  const names=["getItems","getStock","getOpenedConsumables","getLocations","getTransactions","getShoppingList","getShoppingChannels","getShoppingCalendar","getFinancialSummary","getCategories"];
  let path="",fail=false;
  const data={sentinel:true};
  const api=appInternals(names,{getHomeId:()=>"home",t:(key:string)=>key,apiFetch:async(url:string)=>{path=url;return {ok:!fail,json:async()=>data};}});
  ...
  for(const [name,suffix,error,args] of cases){
    fail=false;assert.deepEqual(await api[name](...args),data);assert.equal(path,`/api/v1/homes/home/${suffix}`);
    fail=true;await assert.rejects(api[name](...args),{message:error});
  }
  fail=false;
