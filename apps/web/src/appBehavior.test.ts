...
test("navigation preserves trailing slash, unknown path and detail source behavior", (context) => {
  const browser = {
    location: { pathname: "/" },
    history: { state: null as null | { itemDetailSource: unknown } },
  };
...
test("opened expiry ignores invalid values and compares date-only values at local end of day", () => {
  ...
  assert.equal(
    api.openedExpiryForDisplay({ openedExpiryDate: null, expiryDate: null }),
    null,
  );
  assert.equal(
    api.openedExpiryForDisplay({
      openedExpiryDate: "invalid",
  ...
  assert.equal(
    api.openedExpiryForDisplay({
      openedExpiryDate: "2026-09-02T12:00:00",
      expiryDate: "2026-09-02",
    }),
    "2026-09-02 12:00",
  );
  assert.equal(
    api.openedExpiryForDisplay({
      openedExpiryDate: "2026-09-03T12:00:00",
      expiryDate: "2026-09-02",
    }),
    "2026-09-02",
  );
...
test("read requests preserve paths, paging, snapshot encoding and endpoint errors", async (context) => {
  ...
  context.mock.method(globalThis, "fetch", async (url: RequestInfo | URL) => {
    path = String(url);
    return new Response(JSON.stringify(data), { status: fail ? 400 : 200 });
  });
  ...
  const cases: [
    ...
    ["getItems", "items", "无法加载物资", []],
    ["getStock", "stock", "无法加载库存", []],
    ["getOpenedConsumables", "opened-consumables", "无法加载已开封消耗品", []],
    ["getLocations", "locations", "无法加载地点", []],
    [
      "getTransactions",
    ...
    ["getShoppingList", "shopping-list", "无法加载采购清单", []],
    ["getShoppingChannels", "shopping-channels", "无法加载购买渠道", []],
    [
      "getShoppingCalendar",
      "shopping-calendar?month=2026-09&includeCompleted=false",
      "无法加载采购日历",
      ["2026-09"],
    ],
    [
      "getFinancialSummary",
      "financial-dashboard?month=2026-09",
      "无法加载财务数据",
      ["2026-09"],
    ],
    ["getCategories", "categories", "无法加载物资类型", []],
  ];
  for (const [name, suffix, error, args] of cases) {
    fail = false;
    assert.deepEqual(
      await (api[name] as (...args: unknown[]) => Promise<unknown>)(...args),
      data,
    );
    assert.equal(path, `/api/v1/homes/home/${suffix}`);
    fail = true;
    await assert.rejects(
      (api[name] as (...args: unknown[]) => Promise<unknown>)(...args),
      { message: error },
    );
  }
  fail = false;
  ...
  assert.equal(
    path,
    "/api/v1/homes/home/transactions?limit=20&offset=40&snapshotAt=2026-09-01T00%3A00%3A00%2B08%3A00",
  );
});
