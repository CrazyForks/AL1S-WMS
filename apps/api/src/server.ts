...
app.get<{ Params: { homeId: string } }>("/api/v1/homes/:homeId/items", async (request) => {
  return db.prepare("SELECT items.id, items.home_id AS homeId, items.sku, items.name, items.category, items.base_unit AS baseUnit, items.reorder_point AS reorderPoint, items.reorder_quantity AS reorderQuantity, items.manufactured_date AS manufacturedDate, items.expiry_date AS expiryDate, items.default_location_id AS locationId, locations.name AS locationName, items.active FROM items LEFT JOIN locations ON locations.id = items.default_location_id WHERE items.home_id = ? AND items.active = 1 ORDER BY items.name").all(request.params.homeId);
});

app.get<{ Params: { homeId: string; itemId: string } }>("/api/v1/homes/:homeId/items/:itemId", async (request, reply) => {
  const item = db.prepare("SELECT items.id, items.home_id AS homeId, items.sku, items.name, items.category, items.base_unit AS baseUnit, items.reorder_point AS reorderPoint, items.reorder_quantity AS reorderQuantity, items.manufactured_date AS manufacturedDate, items.expiry_date AS expiryDate, items.default_location_id AS locationId, locations.name AS locationName, items.active FROM items LEFT JOIN locations ON locations.id = items.default_location_id WHERE items.home_id = ? AND items.id = ? AND items.active = 1").get(request.params.homeId, request.params.itemId);
...
app.patch<{ Params: { homeId: string; itemId: string }; Body: unknown }>("/api/v1/homes/:homeId/items/:itemId", async (request, reply) => {
  ...
  return db.prepare("SELECT items.id, items.home_id AS homeId, items.sku, items.name, items.category, items.base_unit AS baseUnit, items.reorder_point AS reorderPoint, items.manufactured_date AS manufacturedDate, items.expiry_date AS expiryDate, items.default_location_id AS locationId, locations.name AS locationName, items.active FROM items LEFT JOIN locations ON locations.id = items.default_location_id WHERE items.id = ? AND items.home_id = ?").get(request.params.itemId, request.params.homeId);
});
...
app.post<{ Params: { homeId: string }; Body: unknown }>("/api/v1/homes/:homeId/items", async (request, reply) => {
  ...
  db.exec("BEGIN");
  try {
    db.prepare("INSERT INTO items (id, home_id, sku, name, category, base_unit, reorder_point, reorder_quantity, default_location_id, manufactured_date, expiry_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(item.id, item.homeId, sku, item.name, item.category, item.baseUnit, item.reorderPoint, item.reorderQuantity, locationId, parsed.data.manufacturedDate ?? null, parsed.data.expiryDate ?? null);
