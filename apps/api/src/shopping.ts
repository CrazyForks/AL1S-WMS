...
type Purchase={id:string;itemId:string|null;name:string;quantity:number;unit:string|null;category:string|null;locationId:string|null;channelId:string|null;plannedDate:string|null;estimatedTotal:number|null;source?:string;completed:number};
export function saveShopping(db:DatabaseSync,homeId:string,raw:unknown,id?:string) {
  ...
  const current=automaticId
    ...
    : id?db.prepare("SELECT id,item_id AS itemId,name,quantity,unit,category,location_id AS locationId,channel_id AS channelId,planned_date AS plannedDate,estimated_total_minor/100.0 AS estimatedTotal,source,completed FROM shopping_list WHERE id=? AND home_id=?").get(id,homeId) as Purchase|undefined:undefined;
  ...
  const estimatedTotalMinor=result.estimatedTotal==null?null:Math.round(result.estimatedTotal*100);
  if(id&&!automaticId) db.prepare("UPDATE shopping_list SET item_id=?,name=?,quantity=?,unit=?,category=?,location_id=?,channel_id=?,planned_date=?,estimated_total_minor=? WHERE id=? AND home_id=?").run(itemId,result.name,result.quantity,result.unit,result.category,result.locationId??null,result.channelId??null,result.plannedDate??null,estimatedTotalMinor,id,homeId);
  else db.prepare("INSERT INTO shopping_list(id,home_id,item_id,name,quantity,unit,category,location_id,channel_id,planned_date,estimated_total_minor,source,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)").run(resultId,homeId,itemId,result.name,result.quantity,result.unit,result.category,result.locationId??null,result.channelId??null,result.plannedDate??null,estimatedTotalMinor,automaticId?"automatic":"manual",new Date().toISOString());
...
export function receiveShopping(db:DatabaseSync,homeId:string,shoppingId:string,raw:unknown) {
  ...
  return withStockOperation(db,homeId,input.idempotencyKey,{type:"purchase",shoppingId,...input},()=>{
    ...
    const row=automatic?db.prepare("SELECT id AS itemId,name,base_unit AS unit,category,default_location_id AS locationId,MAX(reorder_point-(SELECT COALESCE(SUM(CASE WHEN type='receipt' THEN quantity ELSE -quantity END),0) FROM stock_transactions WHERE home_id=? AND item_id=items.id),0) AS quantity,0 AS completed FROM items WHERE home_id=? AND id=? AND active=1").get(homeId,homeId,shoppingId.slice(5)) as Purchase|undefined
      :db.prepare("SELECT id,item_id AS itemId,name,quantity,unit,category,location_id AS locationId,channel_id AS channelId,planned_date AS plannedDate,estimated_total_minor/100.0 AS estimatedTotal,completed FROM shopping_list WHERE home_id=? AND id=?").get(homeId,shoppingId) as Purchase|undefined;
