...
export function listItems(db:DatabaseSync,homeId:string,raw:unknown) {
  ...
  const sql=`SELECT i.icon,i.id,i.home_id AS homeId,i.sku,i.barcode,i.name,i.category,i.base_unit AS baseUnit,i.reorder_point AS reorderPoint,i.reorder_quantity AS reorderQuantity,i.manufactured_date AS manufacturedDate,i.expiry_date AS expiryDate,i.default_location_id AS locationId,l.name AS locationName,i.active,${balance} AS quantity,${latestUnitPrice} AS lastUnitPrice,${latestReceivedAt} AS latestReceivedAt,(SELECT default_currency FROM homes WHERE id=i.home_id) AS currency FROM items i LEFT JOIN locations l ON l.id=i.default_location_id WHERE ${where.join(" AND ")}`;
