...
export function financialDashboard(db:DatabaseSync,homeId:string,raw:unknown) {
  ...
  return {month,currency:home.currency,budgetTotal:budget.total,budgetSourceMonth:budget.sourceMonth,budgetMode:budget.sourceMonth===null?"none":budget.sourceMonth===month?"explicit":"inherited",categoryBudgets,spendingTotal:actual,estimatedTotal:planned,forecastTotal:forecast,remainingBudget:budget.total===null?null:Math.round((budget.total-forecast)*100)/100,variance:actual-planned,inventoryValue:Math.round(inventoryValue*100)/100,pricedBatchCount:known.size,unknownBatchCount:unknown.size,categorySpending:mergeDistribution(actualByCategory,plannedByCategory),byCategory,byChannel,purchases:purchaseRecords,trend,valuation:{byItem:[...valuationByItem.values()].map(({locationIds,...row})=>({...row,locationCount:locationIds.size})).sort((a,b)=>b.value-a.value),byCategory:[...valuationByCategory.values()].sort((a,b)=>b.value-a.value),byLocation:[...valuationByLocation.values()].sort((a,b)=>b.value-a.value)}};
}
