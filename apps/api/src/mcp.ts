...
export function createMcpServer(api: ApiCall, boundHomeId: string | null = null) {
  ...
  register("get_financial_dashboard", "Return complete monthly finance data: total budget position, trend, purchase records, and category execution. Each byCategory row has actual, planned, and budget; budget null is explicitly unbudgeted spending. valuation separates byItem, byCategory, and byLocation.", scoped({month:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/)}), "GET", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/financial-dashboard?${query(value.body)}`};});
  register("set_financial_budget", "Set or clear one month's total budget and category allocations. Confirm the exact amounts with the user immediately before calling: this replaces that month's allocations. Category allocation total cannot exceed total; later months inherit the latest budget until explicitly saved.", scoped({month:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/),total:price.nullable(),categoryBudgets:z.array(z.object({category:name.max(100),amount:price})).max(100)}), "PATCH", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/financial-budget`,body:value.body};});
