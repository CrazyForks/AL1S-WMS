...
export function createMcpServer(api: ApiCall, boundHomeId: string | null = null) {
  ...
  register("get_financial_dashboard", "Return complete monthly finance data: total budget position, trend, purchase records, and category execution. Each byCategory row has actual, planned, and budget. Budgeted parent rows include descendant spending, so do not sum overlapping rows; use dashboard totals. categorySpending contains direct per-category spending; budget null is explicitly unbudgeted spending. valuation separates byItem, byCategory, and byLocation.", scoped({month:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/)}), "GET", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/financial-dashboard?${query(value.body)}`};});
