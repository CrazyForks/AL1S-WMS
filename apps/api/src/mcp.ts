...
export function createMcpServer(api: ApiCall, boundHomeId: string | null = null) {
  ...
  register("confirm_stocktake", "Apply confirmed physical counts atomically. Send the expected quantities from preview; stale counts are rejected. Omit uncounted items.", scoped({locationId,idempotencyKey,rows:z.array(z.object({itemId,expectedQuantity:z.number().nonnegative().finite(),countedQuantity:z.number().nonnegative().finite()})).min(1).max(1000)}), "POST", args=>atHome(args,"/stocktake"));
