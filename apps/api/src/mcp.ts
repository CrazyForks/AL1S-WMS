...
export function createMcpServer(api: ApiCall, boundHomeId: string | null = null) {
  ...
  server.registerTool("get_agent_guide", {
    ...
    pricing:["set estimatedTotal on a shopping item for planning","set totalPrice when receiving the purchase; it is stored on the new batch","use get_financial_summary for monthly spending and inventory value","use get_item_price_history to compare channels and past unit prices"],
  ...
  register("get_financial_summary", "Return actual spending, planned budget, variance, inventory value, unknown-cost batch count, and channel totals for one YYYY-MM month.", scoped({month:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/)}), "GET", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/financial-summary?${query(value.body)}`};});
  register("get_financial_dashboard", "Return the monthly budget position, spending trend, category and channel distributions, purchase records, and inventory valuation.", scoped({month:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/)}), "GET", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/financial-dashboard?${query(value.body)}`};});
  register("set_financial_budget", "Set or clear a monthly total budget and its category allocations. This changes household financial planning data; confirm the amounts with the user first.", scoped({month:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/),total:price.nullable(),categoryBudgets:z.array(z.object({category:name.max(100),amount:price})).max(100)}), "PATCH", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/financial-budget`,body:value.body};});
  ...
  register("receive_shopping_item", "Complete a purchase by receiving actualQuantity as one new batch. totalPrice is the actual amount paid and the purchase channel is inherited from the plan.", scoped({shoppingItemId,actualQuantity:quantity,idempotencyKey,locationId:locationId.optional(),totalPrice:price.optional(),purchaseDate:z.string().date().nullable().optional(),...dates}), "POST", args=>{const value=context(args);const {shoppingItemId,...body}=value.body;return {url:`${homePath(value.homeId)}/shopping-list/${encodeURIComponent(String(shoppingItemId))}/receive`,body};});
