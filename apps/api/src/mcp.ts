...
export function createMcpServer(api: ApiCall, boundHomeId: string | null = null) {
  const server = new McpServer({ name: "AL1S-WMS", version: "0.0.2" });
  ...
  const shoppingFields = {consumptionType:z.enum(["consumable","non_consumable","long_term_consumable"]).optional(),openedShelfLifeDays:z.number().int().positive().nullable().optional(),itemId:itemId.nullable().optional(),name:name.optional(),quantity:quantity.optional(),unit:name.max(30).optional(),category:name.optional(),locationId:locationId.nullable().optional(),channelId:z.string().uuid().nullable().optional(),plannedDate:z.string().date().nullable().optional(),estimatedTotal:price.nullable().optional()};
  ...
  register("receive_shopping_item", "Receive a purchase as a new batch. completion=keep retains any outstanding quantity; complete (default) closes the plan. totalPrice is the actual cost of this delivery, not the full plan.", scoped({shoppingItemId,actualQuantity:quantity,completion:z.enum(["keep","complete"]).optional(),idempotencyKey,locationId:locationId.optional(),totalPrice:price.optional(),purchaseDate:z.string().date().nullable().optional(),...dates}), "POST", args=>{const value=context(args);const {shoppingItemId,...body}=value.body;return {url:`${homePath(value.homeId)}/shopping-list/${encodeURIComponent(String(shoppingItemId))}/receive`,body};});
  ...
  register("transfer_stock", "Transfer stock between locations without changing total stock or batch identity. Omit batchId for FEFO. The server records the source and target as the activity description.", scoped({itemId,sourceLocationId:locationId,targetLocationId:locationId,quantity,idempotencyKey,batchId:z.string().uuid().optional(),openedId:z.string().uuid().optional().describe("Move all or part of this opened record, preserving opening dates")} ), "POST", args=>atHome(args,"/stock/transfers"));
  register("list_stock_operations", "List recent inventory operations and whether they can be undone.", scoped(), "GET", args=>atHome(args,"/stock/operations"));
  register("undo_stock_operation", "Undo a confirmed operation with compensating entries, retaining history. Rejects operations with subsequent changes.", scoped({operationId:z.string().uuid()}), "POST", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/stock/operations/${value.body.operationId}/undo`,body:{}};});
  register("preview_stocktake", "Read stock at one location (excluding children) before counting. Does not change inventory.", scoped({locationId}), "GET", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/stocktake/${value.body.locationId}`};});
  register("preview_category_stocktake", "Read stock for a category and its children, with a separate row for each location. Does not change inventory.", scoped({categoryId:z.string().uuid()}), "GET", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/stocktake/category/${value.body.categoryId}`};});
  register("confirm_stocktake", "Apply confirmed counts for one location or one category atomically. Category rows require locationId. Send expected quantities from preview; stale counts are rejected.", scoped({locationId:locationId.optional(),categoryId:z.string().uuid().optional(),idempotencyKey,rows:z.array(z.object({itemId,locationId:locationId.optional(),expectedQuantity:z.number().nonnegative().finite(),countedQuantity:z.number().nonnegative().finite()})).min(1).max(1000)}), "POST", args=>atHome(args,"/stocktake"));
