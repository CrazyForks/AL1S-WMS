...
export function createMcpServer(api: ApiCall, boundHomeId: string | null = null) {
  ...
  register("get_inventory_cost_analysis", "Analyze inventory cost over an inclusive YYYY-MM range (up to 36 months). Returns inbound purchase cost, normal consumption, expired and damaged waste, adjustments, waste rate, item/category/location waste rankings, unused-versus-partially-used disposal quantities, and current 30-day expiry risk. Issue values use each actual batch's historical unit cost; unknown costs are reported separately.", scoped({start:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/),end:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/)}), "GET", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/inventory-cost-analysis?${query({...value.body,granularity:"month"})}`};});
