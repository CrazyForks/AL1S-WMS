...
export function createMcpServer(api: ApiCall, boundHomeId: string | null = null) {
  ...
  server.registerTool("get_agent_guide", {
    ...
    finance:["use get_financial_dashboard for a month; byCategory contains actual, planned, and budget, where budget null means unbudgeted spending","report category forecast as actual + planned; negative budget remaining is over-budget","valuation.byItem is the default inventory-value view; byCategory and byLocation are separate rollups","call get_financial_dashboard before proposing a budget change and get explicit user confirmation before set_financial_budget","set estimatedTotal on a shopping item for planning and totalPrice when receiving it; the actual cost is stored on the new batch","use get_item_price_history to compare channels and past unit prices"],
  ...
  register("get_financial_summary", "Return compact monthly actual spending, planned spending, inventory value, unknown-cost batch count, and channel totals for one YYYY-MM month. Use get_financial_dashboard for budget execution by category.", scoped({month:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/)}), "GET", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/financial-summary?${query(value.body)}`};});
  ...
  register("get_financial_dashboard", "Return complete monthly finance data: total budget position, trend, purchase records, and category execution. Actual spending follows batch receipt time (received_at), not plannedDate or purchaseDate. Purchase records include receivedDate for financial display. Planned amounts follow planned dates; overdue plans retain their dates until the user reschedules them. Each byCategory row has actual, planned, and budget. Budgeted parent rows include descendant spending, so do not sum overlapping rows; use dashboard totals. categorySpending contains direct per-category spending; budget null is explicitly unbudgeted spending. valuation separates byItem, byCategory, and byLocation.", scoped({month:z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/)}), "GET", args=>{const value=context(args);return {url:`${homePath(value.homeId)}/financial-dashboard?${query(value.body)}`};});
